...
class PriceData:
    def __init__(self, start_date:str='2018-01-01', end_date:str=datetime.today().strftime('%Y-%m-%d')):
    ...
    def get_price_file(self, ticker):
        return f'./data/price/{ticker}.csv'
    
    def download_data(self,ticker):
        print(f'Download data {ticker}')
        data = yf.download(f'{ticker}.JK', start=self.start_date, end=self.end_date, progress=False)
